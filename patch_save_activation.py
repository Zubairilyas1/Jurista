...
new_functions = """
    ...
    try {
      ...
      if (window.showSaveFilePicker) {
        handle = await window.showSaveFilePicker({
          suggestedName: `Petition_${petitionType}.pdf`,
          types: [{
      ...
      if (handle) {
        ...
        a.href = url;
        a.download = `Petition_${petitionType}.pdf`;
        a.click();
  ...
  const handleExportWord = async () => {
    try {
      ...
      if (window.showSaveFilePicker) {
        handle = await window.showSaveFilePicker({
          suggestedName: `Petition_${petitionType}.docx`,
          types: [{
  ...
  const handleExportText = async () => {
    if (editorRef.current) {
      try {
        ...
        if (window.showSaveFilePicker) {
          handle = await window.showSaveFilePicker({
            suggestedName: `Petition_${petitionType}.txt`,
            types: [{
...
print("Patched DocumentEditorModal.tsx to fix User Activation for Save As dialog")
