      ...
      <div className="mb-8 shrink-0 flex items-center justify-between">
        <div>
          ...
          <p className="text-white/50 text-sm font-medium tracking-wider uppercase flex items-center gap-2">
            <Settings size={14} className="text-lime" />
            Lawyer Profile & Preferences
          </p>
        </div>
        
        {saveStatus === 'success' && (
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="flex items-center gap-2 text-lime bg-lime/10 px-4 py-2 rounded-full text-xs font-bold uppercase tracking-widest border border-lime/20">
        ...
        <div className="w-64 shrink-0 flex flex-col gap-2">
          {tabs.map((tab) => (
            <button
              ...
              className={`w-full flex items-center gap-3 px-5 py-4 rounded-xl text-sm font-bold tracking-widest uppercase transition-all ${
                activeTab === tab.id 
                  ? 'bg-lime text-black shadow-[0_0_20px_rgba(204,255,0,0.2)]' 
          ...
          <div className="mt-auto card-dark p-5 rounded-2xl border border-white/5 bg-black/40">
            <h3 className="text-[10px] font-bold text-white/40 uppercase tracking-widest mb-4">Profile Status</h3>
            <div className="flex flex-col gap-3 text-sm">
              <div className="flex items-center justify-between">
                <span className="text-white/60">Advocate Details</span>
                {profile.fullName && profile.barLicense ? (
                  <span className="text-lime flex items-center gap-1"><ShieldCheck size={14}/> Complete</span>
                ) : (
          ...
          <form onSubmit={handleSave} className="max-w-3xl flex flex-col gap-8">
            <AnimatePresence mode="wait">
              <motion.div
                ...
                className="card-dark border-white/10 rounded-2xl overflow-hidden flex flex-col"
              >
                ...
                {activeTab === 'profile' && (
                  <>
                    <div className="p-6 border-b border-white/5 bg-black/40">
                      <h2 className="text-lg font-bold uppercase tracking-widest">Advocate Profile</h2>
                    ...
                    <div className="p-6 flex flex-col gap-6">
                      ...
                      <div className="grid grid-cols-2 gap-6">
                        <div className="flex flex-col gap-2">
                          <label className="text-[10px] font-bold text-lime uppercase tracking-widest">Full Name (with Prefix)</label>
                          <input 
                            value={profile.fullName}
                            onChange={(e) => setProfile({...profile, fullName: e.target.value})}
                            placeholder="e.g. Adv. Muhammad Ali"
                            className="w-full bg-[#121215] border border-white/10 rounded-xl p-4 text-sm font-medium focus:border-lime outline-none transition-colors"
                          />
                        </div>
                        
                        <div className="flex flex-col gap-2">
                          <label className="text-[10px] font-bold text-lime uppercase tracking-widest">Bar License Number</label>
                          <input 
                      ...
                      <div className="grid grid-cols-2 gap-6">
                        ...
                        <div className="flex flex-col gap-2">
                          <label className="text-[10px] font-bold text-white/50 uppercase tracking-widest">Contact Number</label>
                          <input 
                            value={profile.phone}
                ...
                {activeTab === 'firm' && (
                  <>
                    <div className="p-6 border-b border-white/5 bg-black/40">
                      <h2 className="text-lg font-bold uppercase tracking-widest">Firm Details</h2>
                      <p className="text-xs text-white/50 mt-1">Information about your law firm or chamber.</p>
                    </div>
                    <div className="p-6 flex flex-col gap-6">
                      
                      <div className="flex flex-col gap-2">
                        <label className="text-[10px] font-bold text-lime uppercase tracking-widest">Law Firm / Chamber Name</label>
                        <input 
                      ...
                      <div className="flex flex-col gap-2">
                        ...
                        <textarea 
                          ...
                          rows={3}
                          className="w-full bg-[#121215] border border-white/10 rounded-xl p-4 text-sm font-medium focus:border-lime outline-none transition-colors resize-none"
                        />
                      </div>
                    </div>
                  </>
                ...
                {activeTab === 'preferences' && (
                  <>
                    <div className="p-6 border-b border-white/5 bg-black/40">
                      <h2 className="text-lg font-bold uppercase tracking-widest">App Preferences</h2>
                    ...
                    <div className="p-6 flex flex-col gap-6">
                      ...
                      <div className="grid grid-cols-2 gap-6">
                        <div className="flex flex-col gap-2">
                          <label className="text-[10px] font-bold text-lime uppercase tracking-widest">Default Drafting Language</label>
                          <select 
                            value={profile.defaultLanguage}
                            onChange={(e) => setProfile({...profile, defaultLanguage: e.target.value as 'bilingual'|'english'})}
                            className="w-full bg-[#121215] border border-white/10 rounded-xl p-4 text-sm font-medium focus:border-lime outline-none transition-colors appearance-none"
                          >
                        ...
                        <div className="flex flex-col gap-2">
                          <label className="text-[10px] font-bold text-lime uppercase tracking-widest">Default Court</label>
                          <input 
                      ...
                      <div className="flex items-center justify-between p-5 rounded-xl border border-white/5 bg-black/20 mt-4">
                        <div>
            ...
            <div className="flex justify-end">
              <button 
                type="submit"
                disabled={isSaving}
                className="pill-dark px-8 py-4 bg-lime text-black hover:bg-lime/90 font-bold tracking-widest uppercase text-sm flex items-center gap-2 transition-all disabled:opacity-50"
              >
                {isSaving ? (
